...
export class DefaultKernel implements Kernel.IKernel {
  ...
  constructor(options: Kernel.IOptions = {}, id?: string) {
    this._name = options.name || "whatever";
    this._id = id || UUID.uuid4();
    this.serverSettings =
   ...
   */
  private _onWSMessage = (evt: {data: string | ArrayBuffer}) => {
    if (this._wsStopped) {
  ...
  private _ws: PseudoWS | null = null;
  private _username = '';
  ...
  protected _bk_send?: (data: string | ArrayBuffer) => void

  set bk_send(fn: (data: string | ArrayBuffer) => void) {
    this._bk_send = fn
    this._onWSOpen()
  }

  bk_recv(evt: {data: string | ArrayBuffer}): void {
